    ...
    <div className="dash-muted-panel ml-0 mt-3 space-y-4 rounded-[16px] border p-4 sm:ml-12">
      <div>
        <p className="dash-subtle mb-2 text-xs font-semibold uppercase tracking-[0.08em]">
          {t('interviewSetup.sdConfig.duration')}
        </p>
          ...
          {DURATION_OPTIONS.map((minutes) => {
            ...
            return (
              <button
                key={minutes}
                type="button"
                onClick={() => dispatch(setSdConfig({ durationMinutes: minutes }))}
                className={[
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="dash-text text-sm font-semibold">{t('interviewSetup.sdConfig.curveball')}</span>
        </div>
        <ToggleSwitch
          checked={enableCurveball}
          onChange={(value) => dispatch(setSdConfig({ enableCurveball: value }))}
        />
      </div>

      <div className="dash-muted flex items-center gap-2 text-xs">
        <Globe className="h-3.5 w-3.5 shrink-0" />
        {t('interviewSetup.sdConfig.language', {
