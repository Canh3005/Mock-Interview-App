...
function* _handleFetchProbes(action) {
  try {
    ...
    const message = _messageFromError(error, 'Unable to load probes.');
...
function* _handleSaveProbe(action) {
  ...
  try {
    if (id) {
      ...
      toast.success('Draft probe created.');
    }
    yield put(saveProbeSuccess());
    yield put(fetchProbesRequest());
  } catch (error) {
    const message = _messageFromError(error, 'Unable to save probe.');
...
function* _handleTransitionProbe(action) {
  try {
    ...
    toast.success('Probe status updated.');
    yield put(fetchProbesRequest());
  } catch (error) {
    const message = _messageFromError(error, 'Unable to update probe status.');
...
function* _handleFetchInterviewSets(action) {
  try {
    ...
    const message = _messageFromError(error, 'Unable to load interview sets.');
...
function* _handleSaveInterviewSet(action) {
  ...
  try {
    if (id) {
      ...
      toast.success('Draft interview set created.');
    }
    yield put(saveInterviewSetSuccess());
    yield put(fetchInterviewSetsRequest());
  } catch (error) {
    const message = _messageFromError(error, 'Unable to save interview set.');
...
function* _handleTransitionInterviewSet(action) {
  try {
    ...
    toast.success('Interview set status updated.');
    yield put(fetchInterviewSetsRequest());
  } catch (error) {
    const message = _messageFromError(error, 'Unable to update interview set status.');
